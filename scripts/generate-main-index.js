...
function formatTitle(filename) {
  ...
  name = name.replace(
    /^(COMPONENTS|FORMS|LAYOUTS|MEDIA|NAVIGATION|PATTERNS|UTILITIES|ADVANCED)-/,
    '',
  );
...
function generateIndex() {
  ...
  for (const [key, config] of Object.entries(htmlPrefixes)) {
    database[key] = scanDirectory(htmlDir, './[HTML]/Directories', config.label, {
  ...
  if (fs.existsSync(cssCategoriesDir)) {
    const categories = fs
      .readdirSync(cssCategoriesDir, { withFileTypes: true })
  ...
  html = html.replace(
    /href="\.\/components\.html"/g,
    'href="./[HTML]/Directories/components.html"',
  );
  ...
  html = html.replace(
    /href="\.\/navigation\.html"/g,
    'href="./[HTML]/Directories/navigation.html"',
  );
  ...
  if (html.includes(advancedCardMarker)) {
    ...
    html = html.replace(
      ...
      jsCard + cssCard + '<!-- Advanced Components Section -->',
    );
  }
